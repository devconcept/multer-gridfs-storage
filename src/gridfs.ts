...
export class GridFsStorage extends EventEmitter implements StorageEngine {
	...
	constructor(configuration: UrlStorageOptions | DbStorageOptions) {
		...
		const { url, cache, options }: UrlStorageOptions = this.configuration as UrlStorageOptions;
		if (url) {
	 ...
	 **/
	private static async _handleResult(result: FileConfigResult | IteratorResult<FileConfigResult, FileConfigResult>, isGen: boolean): Promise<FileConfigValue> {
		if (isGen) {
			const iteration = result as IteratorResult<FileConfigResult, FileConfigResult>;
			if (iteration.done) {
		...
		return result as FileConfigResult;
	}
	...
	private async fromMulterStream(readStream: NodeJS.ReadableStream, request: Request, file: Express.Multer.File): Promise<GridFile> {
		...
		if (fileSettings === null || fileSettings === undefined) {
			...
			settings = {
				filename: (fileSettings as string | number).toString(),
			};
		} else {
	 ...
	 **/
	private async _generate(request: Request, file: Express.Multer.File): Promise<FileConfigValue> {
		let result: FileConfigResult | IteratorResult<FileConfigResult, FileConfigResult>;
		let isGen = false;
