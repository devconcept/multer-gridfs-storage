...
test.before(async t => {
  ...
  await storage.ready();
  await request(app).post('/options')
...
test('should the parameters be a request and a file objects', t => {
  ...
  params.forEach((p) => {
    const {req, file} = p;
    t.is(req, appReq);
    ['body', 'query', 'params', 'files'].every(k => t.true(req.hasOwnProperty(k)));
    ['fieldname', 'originalname', 'encoding', 'mimetype'].every(k => t.true(file.hasOwnProperty(k)));
  });
});
