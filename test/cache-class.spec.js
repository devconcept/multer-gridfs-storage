...
test('cache initializes with a url and a cache name and no connection options', t => {
	...
	t.not(cache._connections.b[url], undefined);
	t.deepEqual(cache._connections.b[url]['0'], {
		db: null,
		client: null,
		pending: true,
		opening: false,
...
test('new cache is created if the same url and different options are used', t => {
	...
	t.deepEqual(cache._connections.b[url]['1'], {
		db: null,
		client: null,
		pending: true,
		opening: false,
...
function cachesShouldBeDifferent(t, firstUrl, secondUrl) {
	const {cache} = t.context;
	cache.initialize({url: firstUrl, cacheName: 'a'});
	cache.initialize({url: secondUrl, cacheName: 'a'});
	t.not(cache._connections.a, undefined);
	t.not(cache._connections.a[firstUrl], undefined);
	t.deepEqual(cache._connections.a[firstUrl]['0'], {
		db: null,
		client: null,
		pending: true,
		opening: false,
	...
	t.is(cache._connections.a[firstUrl]['1'], undefined);
	t.not(cache._connections.a[secondUrl], undefined);
	t.deepEqual(cache._connections.a[secondUrl]['0'], {
		db: null,
		client: null,
		pending: true,
		opening: false,
...
function cachesShouldBeEqual(t, firstUrl, secondUrl) {
	...
	if (firstUrl !== secondUrl) {
		t.is(cache._connections.a[secondUrl], undefined);
	}
