...
test('compare includes buffers when comparing', (t) => {
	...
	t.false(compare({ a: { b: Buffer.from([1, 2]) } }, { a: { b: Buffer.from([2, 2]) } }));
});

test('compare includes buffers inside arrays when comparing', (t) => {
	t.true(compare({ a: { b: ['1', Buffer.from([1, 2])] } }, { a: { b: ['1', Buffer.from([1, 2])] } }));
	t.false(compare({ a: { b: ['1', Buffer.from([1, 2])] } }, { a: { b: ['1', Buffer.from([2, 2])] } }));
});
