...
try {
  // If mongodb is installed using git or some other way with `npm install` requiring it will throw an exception
  // This try-catch allows to continue execution and
...
GridFSStorage.prototype._connect = function _connect(opts) {
  ...
  if (!opts.db) {
    ...
    if (opts.db.then) {
      ...
      self.db = opts.db;
      self.connected = self.db.serverConfig.isConnected();
...
GridFSStorage.prototype._handleResult = function _handleResult(result, cb, isGen) {
  ...
  if (isPromise(value)) {
    value.then(function onFulfill(data) {
      cb(null, data);
