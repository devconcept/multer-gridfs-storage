...
GridFSStorage.prototype._connect = function _connect() {
	...
	if (!db) {
		if (this.caching) {
			if (!GridFSStorage.cache.isOpening(this.cacheIndex) && GridFSStorage.cache.isPending(this.cacheIndex)) {
...
GridFSStorage.prototype._createConnection = function _createConnection() {
	...
	MongoClient.connect(url, options, (err, _db) => {
		...
		if (err) {
			return this.cacheIndex ? GridFSStorage.cache.reject(this.cacheIndex, err) : this._fail(err);
		}
