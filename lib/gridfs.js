...
function GridFSStorage(configuration) {
	...
	if (this.caching) {
		this.cacheName =
			typeof configuration.cache === 'string' ? configuration.cache : 'default';
	...
	if (!db) {
		if (this.caching) {
			if (
				...
				cached = GridFSStorage.cache.get(this.cacheIndex);
				cached.opening = true;
 ...
 **/
GridFSStorage.prototype._createConnection = function() {
	let db;
	let client = null;
	...
	MongoClient.connect(url, options, (err, _db) => {
		...
		if (err) {
			return this.cacheIndex
				? GridFSStorage.cache.reject(this.cacheIndex, err)
				: this._fail(err);
		}
 ...
 **/
GridFSStorage.prototype._updateConnectionStatus = function() {
	if (!this.db) {
		this.connected = false;
		this.connecting = false;
		return;
 ...
 **/
GridFSStorage.prototype._setDb = function(db, client) {
	this.connecting = false;
 ...
 **/
GridFSStorage.prototype._fail = function(err) {
	this.db = null;
	this.client = null;
	this.error = err;
	this._updateConnectionStatus();
...
GridFSStorage.prototype._handleFile = function(req, file, cb) {
	...
	if (this.connecting) {
		...
		this.once('connection', connectionListener).once(
			'connectionFailed',
			failedListener
		);
	} else {
 ...
 * @private
 */
GridFSStorage.prototype._store = function(req, file, cb) {
 ...
 **/
GridFSStorage.prototype._generate = function(req, file) {
	let result;
	let generator;
 ...
 *
 **/
GridFSStorage.prototype._handleResult = function(result, isGen) {
	let value = result;
 ...
 */
GridFSStorage._mergeProps = function(extra, fileSettings) {
	let promise;
 ...
 */
GridFSStorage.prototype.ready = function() {
	if (this.error) {
