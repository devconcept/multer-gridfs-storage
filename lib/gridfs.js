...
GridFSStorage.prototype._connect = function _connect(opts) {
  var self = this;
  var promise, url, cached;

  if (!opts.db) {
    url = opts.url;
    if (self.caching) {
      if (!cache.isOpening(self.cacheIndex) && cache.isPending(self.cacheIndex)) {
        cached = cache.get(self.cacheIndex);
        cached.opening = true;
        self._createConnection(url, opts.connectionOpts);
      }
      cache
        .waitFor(self.cacheIndex)
        .then(function (cached) {
          self._setDb(cached.db, cached.client);
        })
        .catch(function (err) {
          self._fail(err);
        });
 ...
 * @param {boolean|string} cacheName - The name of the cache to store the connection
 *
 **/
GridFSStorage.prototype._createConnection = function _createConnection(url, connectionOpts) {
  ...
  self.connecting = true;
  MongoClient.connect(url, connectionOpts, function (err, _db) {
    var parsedUri;

    if (err) {
      return self.cacheIndex ? cache.reject(self.cacheIndex, err) : self._fail(err);
    }
    ...
    if (self.caching) {
      cache.resolve(self.cacheIndex, db, client);
    } else {
      self._setDb(db, client);
    }
  });
};
...
GridFSStorage.prototype._setDb = function _setDb(db, client) {
  ...
  process.nextTick(function () {
    self.emit('connection', self.db, self.client);
  });
};
...
GridFSStorage._mergeProps = function _mergeProps(extra, fileSettings) {
  ...
  if (!fileSettings.filename) {
    ...
    randomBytes = utils.isOldNode() ? crypto.pseudoRandomBytes : crypto.randomBytes;
  ...
  return promise
    ...
    .then(function (prev) {
      return typeof Object.assign === 'function' ?
        Object.assign(prev, defaults, extra, fileSettings) :
        utils.assign(prev, defaults, extra, fileSettings);
    });
};
