...
The GridFS instance to use.

If this connection is not opened the module might fail to store incoming files since this
module does not test for an existing connection. This option is useful when you have
an existing GridFS object and want to reuse it to upload your files.
...
With this option the module will create a GridFS instance for you instead. 
...
storage.on('connection', function(gfs, db) {
   // Do something with the GridFS or the MongoDb instance
   console.log('MongoDb connected in url ' + url);
});
```

Note that if you only want to log events there is an api option for that
