...
------------- | -----------
`filename` | The desired filename for the file (required)
`id` | An ObjectID to use as identifier (autogenerated if not present)
...
`contentType` | The content type for the file (inferred from the request if absent)
